...
bool judge(string str, int line_count){// 判断是否为数字/标识符/关键字
    ...
    for (auto & i : keyword) {
        if (str == i.first){
            outFile <<"( " << i.second << " , - )";
            outFile<<" curr line : " << line_count << endl;
    ...
    if ((str[0] >= 'a' and str[0] <= 'z')
        ...
        outFile << "( 71 , " << identifier_mp.size() << " )" << " curr line : " << line_count << endl;
        ...
        if (p_temp.first == 0 and p_temp.second == 0){
            ...
            outFile << "( 68 , "<< constTable.size() << " )" << " curr line : " << line_count << endl; //整数
